    ...
    const {loader} = useSelector(state=>state.auth)
     ...
     const profileImageHandler = (e) => {
      ...
      if (file) {
        ...
        reader.onloadend = () => {
          setUserData({ ...userData, profileImage: file }); // Keep this line intact
        };
            ...
            <div className="flex justify-center h-[250px] w-[250px]  rounded-md">
            {
               userData.profileImage ? (
                  <label htmlFor='img' className='h-full w-full relative cursor-pointer overflow-hidden rounded-md'>
                    <img className='w-full h-full object-cover rounded-full' src={URL.createObjectURL(userData.profileImage)} alt="Profile" />
                    {loader && (
