...
const AddListing = () => {
    ...
    const featureSearch = (e) => {
        ...
        setSearchValue(value);
        if (value) {
            const filtered = additionalFeatures.filter((item) =>
                item.name.toLowerCase().includes(value.toLowerCase())
            );
            setAllFeatures(filtered);
        } else {
            setAllFeatures(additionalFeatures);
        }
    };
    ...
    // console.log(e.target.files)
     ...
     const handleFeatureToggle = (featureName) => {
        ...
        if (selectedFeatures.includes(featureName)) {
            // Deselect feature
            setSelectedFeatures((prev) => prev.filter((item) => item !== featureName));
   ...
   const add = (e)=>{
    ...
    const formData = new FormData()
        ...
        formData.append("additionalFeatures", JSON.stringify(selectedFeatures));
                ...
                <form onSubmit={add}>
                    <div className="flex flex-col mb-3 md:flex-row gap-4 w-full text-text_color">
                        <div className="flex flex-col w-full gap-3">
                            ...
                            <div className="w-full relative">
                                <label htmlFor="category">Listing Features</label>
                                <input
                                    readOnly
                                    onClick={() => setShowFeature(!showFeature)}
                                    value={selectedFeatures.join(', ')} // Display selected features
                                    ...
                                    type="text"
                                    placeholder="Select Listing Features"
                                    name="additionalFeatures"
                                    id="additionalFeatures"
                                />
                                <div
                                    className={`absolute top-[100%] bg-slate-800 w-full transition-all z-[99999] px-4 ${
                                        showFeature ? 'scale-100' : 'scale-0'
                                    }`}
                                >
                                    <div className="w-full px-4 py-2">
                                        <input
                                            value={searchValue}
                                            onChange={featureSearch}
                                            className="w-full px-3 py-1 focus:border-primary outline-none bg-transparent border-2 border-slate-700 rounded-md text-[#d0d2d6]"
                                            type="text"
                                            placeholder="Search"
                                        />
                                    </div>
                                    <div className="py-4 border-b-2 border-slate-500 mb-2"></div>
                                        ...
                                        {allFeatures.map((feature, i) => (
                                            ...
                                            >
                                                <input
                                                    type="checkbox"
                                                    className="mr-2"
                                                    checked={selectedFeatures.includes(feature.name)} // Check if feature is selected
                            ...
                            <div className="w-8/12 flex-row flex relative justify-between pt-3">
                                <div className="w-full">
                                    ...
                                    <input
                                        ...
                                        type="number"
                                        placeholder="Price per Unit per Kilometer"
                                        min="0" // Ensures no negative values
                                        step="any" // Allows decimals and values less than 1
                                        name="pricePerUnit"
                                        id="pricePerUnit"
                                    />
                                    </div>
