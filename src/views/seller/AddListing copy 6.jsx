                    ...
                    <div className="flex flex-col mb-3 md:flex-row gap-4 w-full text-text_color">
                        <div className="flex flex-col w-full gap-3">
                            ...
                            <div className="w-full relative">
                                ...
                                <input readOnly onClick={()=>setShowFeature(!showFeature)} onChange={inputHandle} value={feature} className='w-full bg-transparent px-4 py-2 focus:border-accent outline-none bg-[#283046] border-2 border-slate-700 rounded-md text-[#d0d2d6]' type="text" placeholder='Select Listing Category'  name='additionalFeature' id='additionalFeatures'/>
                                <div className={`absolute top-[100%]  bg-slate-800 w-full transition-all z-[99999] px-4  ${showFeature ? 'scale-100':'scale-0'}`}>
                                    <div className="w-6/12 px-4 py-2 fixed right-1 top-1 ">
                                            <input value={searchValue} onChange={featureSearch}  className='w-full self-end flex flex-col items-start justify-center px-3 py-1 overflow-hidden focus:border-primary outline-none bg-transparent border-2 border-slate-700 rounded-md text-[#d0d2d6]' type="text" placeholder='search' />
                                    </div>
                                    <div className="py-8 border-b-2 border-slate-500 mb-2"></div>
                                    <div className="flex justify-start items-start flex-col h-[200px] overflow-x-hidden">
                                            {
                                                allFeatures.map(( allFeatures,i) => 
                                                <span key={i} className={`px-4 py-2 hover:bg-primary hover:text-text_color rounded-md w-full cursor-pointe my-1 ${feature ===  allFeatures.name && 'bg-primary'}`} onClick={()=>{
                                                    // setShowFeature(false)
                                                    setFeature(allFeatures.name)
                                                    setSearchValue('')
                                                    setAllFeatures(additionalFeatures)
