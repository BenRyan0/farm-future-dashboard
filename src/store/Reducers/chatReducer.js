...
export const get_admin_message = createAsyncThunk(
    ...
    const config = {
      headers : {
        ...
        try {
            const { data } = await api.get(`/chat/get-admin-messages/${receiverId}`, config)
            console.log(data)
            return fulfillWithValue(data)
        } catch (error) {
            return rejectWithValue(error.response.data)
...
export const chatReducer = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(get_customer_message.fulfilled, (state, payload) => {
          ...
          state.currentCustomer = payload.payload.currentCustomer;
        });
    
        builder.addCase(send_message.fulfilled, (state, payload) => {
            let tempFriends = state.customers
            let index = tempFriends.findIndex(f => f.fdId === payload.payload.message.receiverId)
            while (index > 0) {
            ...
            state.messages = [...state.messages, payload.payload.message]
          ...
          builder.addCase(send_message_seller_admin.fulfilled, (state, payload) => {
            state.seller_admin_message = [...state.seller_admin_message, payload.payload.message];
            state.successMessage = 'Message Sent'
          });

          builder.addCase(get_admin_message.fulfilled, (state, payload) => {
            state.seller_admin_message = payload.payload.messages;
